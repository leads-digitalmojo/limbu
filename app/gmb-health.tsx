import { useRouter } from 'expo-router';
import React, { useEffect, useState } from 'react';
import { View } from 'react-native';
import { Donut } from '../components/charts';
import { Icon, IconName } from '../components/Icon';
import {
  Badge, Between, Button, Card, CardBody, CardHead, Chip, Cols, IconTile, Muted, PageHeader,
  Progress, Row, Select, Stack, T, useWork,
} from '../components/ui';
import { gmbApi, GmbProfile } from '../lib/api/gmb';
import { useBiz, useStore } from '../store/useStore';
import { useUI } from '../store/ui';
import { useTheme } from '../theme/ThemeProvider';

type Check = { label: string; w: number; score: number; note: string; simulated: boolean };

// Photos & videos and Post frequency have no public API left to check for
// real: the Business Profile Photos/Media API was sunset for third-party
// apps in 2022, and Local Posts has been invite-only since the same year.
// Services & pricing and Attributes & amenities are real Google APIs, but
// scoring them meaningfully needs a per-category metadata lookup (the full
// list of *available* attributes for "Dental clinic", say) that isn't built
// yet. All four stay simulated no matter what's connected.
const SIMULATED: Record<string, { score: number; note: string }> = {
  'Photos & videos': { score: 45, note: 'Only 11 photos. Profiles with 30+ get 42% more direction requests.' },
  'Services & pricing': { score: 70, note: '6 of 11 services listed. Add prices to improve conversions.' },
  'Post frequency': { score: 55, note: 'Last post 9 days ago. Weekly posting is the ranking sweet spot.' },
  'Attributes & amenities': { score: 40, note: 'Add wheelchair access, parking and payment options.' },
};

function buildChecks(profile: GmbProfile | null, pendingReplies: number, totalReviews: number): Check[] {
  const nameScore = !profile ? 100 : profile.title && profile.primaryCategory ? 100 : (profile.title || profile.primaryCategory) ? 50 : 0;
  const nameNote = !profile ? 'Primary category "Dental clinic" matches your services.'
    : profile.title && profile.primaryCategory ? `"${profile.primaryCategory}" is set as the primary category.`
    : 'Add a business name and a primary category on Google.';

  const secCount = profile?.additionalCategories.length ?? 2;
  const secScore = !profile ? 60 : Math.min(100, secCount * 34);
  const secNote = !profile ? 'Add "Cosmetic dentist" and "Orthodontist" to appear in more searches.'
    : secCount === 0 ? 'No secondary categories set — add a few to appear in more searches.'
    : `${secCount} secondary categor${secCount === 1 ? 'y' : 'ies'} set: ${profile.additionalCategories.join(', ')}.`;

  const hoursScore = !profile ? 100 : profile.hasHours ? 100 : 0;
  const hoursNote = !profile ? 'Complete, including special holiday hours.'
    : profile.hasHours ? 'Regular hours are set on your profile.' : 'No business hours set on Google.';

  const descLen = profile?.description?.length ?? 0;
  const descScore = !profile ? 80 : descLen >= 250 ? 100 : descLen > 0 ? 60 : 0;
  const descNote = !profile ? 'Good, but missing your top keyword "dentist near me".'
    : descLen === 0 ? 'No business description set.'
    : descLen >= 250 ? 'A full-length description is set.' : `Only ${descLen} characters — Google allows up to 750.`;

  const siteScore = !profile ? 100 : profile.websiteUri ? 100 : 0;
  const siteNote = !profile ? 'Website and appointment link are both live.'
    : profile.websiteUri ? profile.websiteUri : 'No website URL set on your profile.';

  const replyRate = totalReviews === 0 ? 1 : (totalReviews - pendingReplies) / totalReviews;
  const reviewScore = Math.round(replyRate * 100);
  const reviewNote = pendingReplies === 0 ? 'Every review currently has a reply.'
    : `${pendingReplies} review${pendingReplies === 1 ? ' is' : 's are'} still waiting for a reply.`;

  return [
    { label: 'Business name & category', w: 10, score: nameScore, note: nameNote, simulated: !profile },
    { label: 'Secondary categories', w: 8, score: secScore, note: secNote, simulated: !profile },
    { label: 'Business hours', w: 9, score: hoursScore, note: hoursNote, simulated: !profile },
    { label: 'Photos & videos', w: 14, ...SIMULATED['Photos & videos'], simulated: true },
    { label: 'Services & pricing', w: 12, ...SIMULATED['Services & pricing'], simulated: true },
    { label: 'Business description', w: 8, score: descScore, note: descNote, simulated: !profile },
    { label: 'Post frequency', w: 14, ...SIMULATED['Post frequency'], simulated: true },
    // always derived from the reviews already in the store — real once review sync
    // exists, demo seed data until then, but never a fabricated constant.
    { label: 'Reviews & replies', w: 15, score: reviewScore, note: reviewNote, simulated: false },
    { label: 'Attributes & amenities', w: 5, ...SIMULATED['Attributes & amenities'], simulated: true },
    { label: 'Website & booking link', w: 5, score: siteScore, note: siteNote, simulated: !profile },
  ];
}

const PLAN_BASE: [IconName, string, string][] = [
  ['image', 'Upload more photos', '/assets'],
  ['send', 'Publish posts more often', '/posts/new'],
  ['tag', 'Add missing services', '/gmb-health'],
  ['key', 'Work a top keyword into your description', '/keywords'],
];

export default function GmbHealth() {
  const { c } = useTheme();
  const router = useRouter();
  const biz = useBiz();
  const { businesses, activeBiz, setActiveBiz, reviews } = useStore();
  const { toast } = useUI();
  const { run, isBusy } = useWork();

  const [profile, setProfile] = useState<GmbProfile | null>(null);
  const [source, setSource] = useState<'live' | 'demo'>('demo');

  const pendingReplies = reviews.filter((r) => r.bizId === biz.id && !r.reply).length;
  const totalReviews = reviews.filter((r) => r.bizId === biz.id).length;

  async function load() {
    if (biz.googleLocationId) {
      try {
        const live = await gmbApi.getProfile(biz.googleLocationId);
        setProfile(live);
        setSource('live');
        return;
      } catch {
        toast('Could not load live profile data', 'Showing demo data instead', 'err');
      }
    }
    setProfile(null);
    setSource('demo');
  }

  useEffect(() => { load(); }, [biz.googleLocationId]);

  const CHECKS = buildChecks(profile, pendingReplies, totalReviews);
  const score = Math.round(
    CHECKS.reduce((a, x) => a + x.score * x.w, 0) / CHECKS.reduce((a, x) => a + x.w, 0));
  const missing = CHECKS.filter((x) => x.score < 80);
  const scoreColor = score >= 80 ? c.emerald : score >= 60 ? c.lemonHover : c.red;

  const PLAN: [IconName, string, string, string][] = [
    ...PLAN_BASE.map(([icon, title, href]) => [icon, title, '', href] as [IconName, string, string, string]),
  ];
  if (pendingReplies > 0) {
    PLAN.splice(2, 0, ['reply', `Reply to ${pendingReplies} pending review${pendingReplies === 1 ? '' : 's'}`, '', '/review-reply']);
  }

  return (
    <View>
      <PageHeader eyebrow="Profile audit" eyebrowIcon="stethoscope" title="GMB Health"
        sub={`A full audit of ${biz.name} against the 10 signals Google weighs most for local ranking.`}
        actions={
          <>
            {source === 'live'
              ? <Badge label="Partially live" tone="green" icon="checkCircle" />
              : <Badge label="Demo data" tone="amber" />}
            <View style={{ width: 220 }}>
              <Select value={activeBiz} onChange={setActiveBiz} options={businesses.map((b) => ({ value: b.id, label: b.name }))} />
            </View>
            <Button label="Run health check" variant="primary" icon="refresh" loading={isBusy('run')}
              onPress={() => run('run', 1800, () => { load(); toast('Health check complete', 'Audited 10 profile signals', 'ok'); })} />
          </>
        } />

      <Cols
        sideWidth={320}
        main={
          <>
            <Card>
              <CardHead title="Profile audit" sub="Weighted by ranking impact" />
              <CardBody>
                <Stack gap={16}>
                  {CHECKS.map((x) => (
                    <View key={x.label}>
                      <Between style={{ marginBottom: 6 }}>
                        <Row gap={8} wrap={false} style={{ flexShrink: 1 }}>
                          <Icon name={x.score >= 80 ? 'checkCircle' : 'alert'} size={16}
                            color={x.score >= 80 ? c.emerald : x.score >= 60 ? c.lemonHover : c.red} />
                          <T size={13} weight="700">{x.label}</T>
                          <Badge label={`weight ${x.w}`} />
                          {x.simulated && <Chip label="Simulated" />}
                        </Row>
                        <T size={12} weight="700">{x.score}%</T>
                      </Between>
                      <Progress value={x.score} tone={x.score >= 80 ? 'green' : x.score >= 60 ? 'lemon' : 'red'} />
                      <Muted size={11.5} style={{ marginTop: 6 }}>{x.note}</Muted>
                    </View>
                  ))}
                </Stack>
              </CardBody>
            </Card>

            <Card>
              <CardHead title="Optimisation plan" sub="Do these in order for the fastest gain" />
              <CardBody>
                <Stack gap={10}>
                  {PLAN.map(([icon, title, , href]) => (
                    <Card key={title} pad={11} onPress={() => router.push(href as any)}>
                      <Between>
                        <Row gap={10} wrap={false} style={{ flexShrink: 1 }}>
                          <IconTile icon={icon} size={30} />
                          <T size={13} weight="600" style={{ flexShrink: 1 }}>{title}</T>
                        </Row>
                        <Row gap={6} wrap={false}>
                          <Badge label="Recommended" tone="green" />
                          <Icon name="chevronR" size={15} color={c.text3} />
                        </Row>
                      </Between>
                    </Card>
                  ))}
                </Stack>
              </CardBody>
            </Card>
          </>
        }
        side={
          <>
            <Card pad={20}>
              <Donut value={score} max={100} color={scoreColor} label="Health score" />
              <View style={{ alignItems: 'center', marginTop: 12 }}>
                <Badge tone={score >= 80 ? 'green' : score >= 60 ? 'amber' : 'red'}
                  label={score >= 80 ? 'Strong profile' : score >= 60 ? 'Needs work' : 'Critical issues'} />
                <Muted style={{ textAlign: 'center', marginTop: 10 }}>
                  Fixing the {missing.length} items below could lift you an estimated +{100 - score} points.
                </Muted>
              </View>
            </Card>

            <Card>
              <CardHead title="Missing information" />
              <CardBody>
                <Stack gap={10}>
                  {missing.map((x) => (
                    <Row key={x.label} gap={9} wrap={false}>
                      <Icon name="alert" size={15} color={c.red} />
                      <T size={12.5} style={{ flex: 1 }}>{x.label}</T>
                      <Badge label={`${x.score}%`} tone="red" />
                    </Row>
                  ))}
                </Stack>
              </CardBody>
            </Card>
          </>
        }
      />
    </View>
  );
}
